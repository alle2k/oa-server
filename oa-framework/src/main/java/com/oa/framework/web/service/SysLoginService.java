...
import com.oa.common.exception.user.*;
...
import com.oa.framework.security.context.AuthenticationContextHolder;
...
public class SysLoginService {
     ...
     * @return 结果
     */
    public String login(String username, String password, String code, String uuid, String recaptcha) {
        // 验证码校验
    ...
    private void validateRecaptcha(String recaptcha) {
        if (StringUtils.isBlank(recaptcha)) {
            return;
        }
        Map<String, Object> map = new HashMap<>();
        map.put("secret", secret);
        map.put("response", recaptcha);
        JSONObject jsonObject = JSON.parseObject(HttpUtil.post(url, map));
        log.info("GOOGLE RECAPTCHA RESPONSE:{}", jsonObject);
